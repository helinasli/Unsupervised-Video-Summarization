...
class Solver(object):
    # Configuration, Train Loader (DataLoader) , TestLoader (VideoData)
    def __init__(self, config=None, train_loader =None, test_loader =None):
        ...
        # Build Modules
        # Linear Compress feature vector boyutunu düşürür. Linear dönüşüm uygular.
        # Arguman olarak input ve putput size alır
        self.linear_compress = nn.Linear(self.config.input_size, self.config.hidden_size).cuda()
        ...
        self.model = nn.ModuleList([self.linear_compress, self.summarizer, self.discriminator])

        if self.config.mode == 'train':
            # Build Optimizers
            self.s_e_optimizer = optim.Adam(
                list(self.summarizer.attn.parameters()) + list(self.summarizer.vae.e_lstm.parameters())
                + list(self.linear_compress.parameters()),
                lr=self.config.lr)

            self.d_optimizer = optim.Adam(
                list(self.summarizer.vae.d_lstm.parameters()) + list(self.linear_compress.parameters()),
                lr=self.config.lr)
