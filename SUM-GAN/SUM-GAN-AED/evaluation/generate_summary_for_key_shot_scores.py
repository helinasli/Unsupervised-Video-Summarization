...
def generate_summary_for_key_shot_scores(all_shot_bound, all_scores, all_nframes, all_positions): 
    all_summaries = []
    for video_index in range(len(all_scores)):
        ...
        if positions[-1] != n_frames:
            ...
            print('WOOOW',n_frames,len(positions),len(frame_init_scores))
        for i in range(len(positions) - 1):
            pos_left, pos_right = positions[i], positions[i+1]
            if i == len(frame_init_scores):     #an einai to last frame 
